...
function loadWidget(config) {
	...
	(function welcomeMessage() {
		let text;
		if (location.pathname === "/") { // 如果是首頁
			const now = new Date().getHours();
			if (now > 5 && now <= 7) text = "早安！一日之計在於晨，美好的一天就要開始了。";
			else if (now > 7 && now <= 11) text = "早安！工作順利嘛，不要久坐，多起來走動走動哦！";
			else if (now > 11 && now <= 13) text = "中午了，工作了一個上午，現在是午餐時間！";
			else if (now > 13 && now <= 17) text = "午後很容易想睡呢，今天的運動目標完成了嗎？";
			else if (now > 17 && now <= 19) text = "傍晚了！窗外夕陽的景色很美麗呢，最美不過夕陽紅～";
			else if (now > 19 && now <= 21) text = "晚安，今天過得怎麼樣？";
			...
			else if (domain === "google") text = `Hello！來自 Google搜索 的朋友<br>歡迎閱讀<span>「${document.title.split(" - ")[0]}」</span>`;
